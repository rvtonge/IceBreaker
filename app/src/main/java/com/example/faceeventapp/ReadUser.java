 ...
 */
public class ReadUser extends AsyncTask<User,Void,User> {
    ...
    public static User checkLogin(User user) throws Exception
    {
       ...
       {
           if(tempAttribute.get(i).getName().equals("password"))
           {
               if(user.password.equals(tempAttribute.get(i).getValue()))
               {
                   profile.password = tempAttribute.get(i).getValue();
               }
               else{
                   profile.password = "Invalid Password";
                   return profile;
               }
           }
